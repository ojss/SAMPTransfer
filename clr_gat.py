...
class CLRGAT(pl.LightningModule):
    ...
    def mpnn_forward_pass(self, x_support, x_query, y_support, y_query, ways):
        loss_cnn = 0.
        _, z_orig, (z,) = self.forward(torch.cat([x_support, x_query]),
    ...
    def supervised_finetuning(self, episode, device='cpu', proto_init=True,
        ...
        if self.mpnn_opts["adapt"] == "task":
            ...
            combined = self.backbone(combined).flatten(1)
        ...
        if freeze_backbone is False:
            delta_opt = torch.optim.Adam(
                filter(lambda p: p.requires_grad, self.parameters()), lr=inner_lr)
