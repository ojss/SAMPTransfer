...
class ProtoClassifier():
    def __init__(self, n_way, n_support, n_query, ot=True, dev="cuda"):
        self.n_way = n_way
    ...
    def __call__(self, z_support, y_support, z_query):
        ...
        if self.ot:
            ...
            z_support, z_query = transportation_module(z_support, z_query)
...
def finetune(novel_loader, n_query=15, freeze_backbone=False,
             ...
             pretrained_dataset='miniImagenet', proto_init=False, ot=True):
    correct = 0
    count = 0
    ...
    with tqdm(enumerate(novel_loader), total=len(novel_loader)) as pbar:
        ...
        for _, (x, y) in pbar:  # , position=1,
            ...
            checkpoint_dir = '%s/checkpoints/%s/%s_%s_%s%s_%s%s' % (configs.save_dir, params.dataset,
                                                                    ...
                                                                    params.n_support,
                                                                    "s" if params.no_aug_support else "s_aug",
                                                                    params.n_query,
                                                                    "q" if params.no_aug_query else "q_aug")
            ...
            x_a_i = x_var[:, :n_support, :, :, :].contiguous().view(n_way * n_support,
