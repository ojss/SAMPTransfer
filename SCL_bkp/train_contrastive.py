...
def main():
    ...
    if opt.spatial_cont_loss:
        attention = AttentionSimilarity(hidden_size=model.encoder.feat_dim, inner_size=opt.feat_dim,
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        time1 = time.time()
        train_loss = train(epoch, train_loader, model, criterion_cls, criterion_contrast, criterion_contrast_spatial,
                           attention, optimizer, opt)
        time2 = time.time()
...
def train(epoch, train_loader, model, criterion_cls, criterion_contrast, criterion_contrast_spatial, attention,
          optimizer, opt):
    """
    One training epoch
    """
