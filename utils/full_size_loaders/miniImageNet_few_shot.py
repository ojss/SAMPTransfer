...
class SimpleDataset:
    def __init__(self, transform, original_transform, miniImageNet_path,
...
class SetDataset:
    def __init__(self, batch_size, transform, split, miniImageNet_path):
        klasses = {
            'train': 64,
            'val': 16,
            'test': 20
        ...
        self.sub_meta = {}
        self.cl_list = range(klasses[split])
...
class SimpleDataManager(DataManager):
    ...
    def get_data_loader(self, aug, n_support=1, n_query=1, no_aug_support=False,
                        no_aug_query=False):  # parameters that would change on train/val set
...
class SetDataManager(DataManager):
    def __init__(self, image_size, split, miniImageNet_path, n_way=5, n_support=5, n_query=16, n_eposide=100):
        ...
        self.miniImageNet_path = miniImageNet_path

        self.trans_loader = TransformLoader(image_size)

    def get_data_loader(self, aug):  # parameters that would change on train/val set
        transform = self.trans_loader.get_composed_transform(aug)
        dataset = SetDataset(self.batch_size, transform, self.split, self.miniImageNet_path)
